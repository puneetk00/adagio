...
namespace ET\Promotionbar\Model\ResourceModel;
...
class Promotion extends AbstractDb {
    ...
    protected function _beforeSave(AbstractModel $object) {
        if (!$this->getIsUniquePromotionToStores($object)) {
     ...
     * Load an object
     *
     * @param \ET\Promotionbar\Model\Promotion|AbstractModel $object
     * @param mixed $value
    ...
    protected function _getLoadSelect($field, $value, $object) {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                            ['cbs' => $this->getTable('et_promotionbar_store')], $this->getMainTable() . '.' . $linkField . ' = cbs.' . $linkField, ['store_id']
                    )
                    ->where('is_active = ?', 1)
     ...
     */
    public function getIsUniquePromotionToStores(AbstractModel $object) {
        $entityMetadata = $this->metadataPool->getMetadata(PromotionInterface::class);
        $linkField = $entityMetadata->getLinkField();
        ...
        $select = $this->getConnection()->select()
                ...
                ->join(
                        ['cbs' => $this->getTable('et_promotionbar_store')], 'cb.' . $linkField . ' = cbs.' . $linkField, []
                )
    ...
    public function lookupStoreIds($id) {
        ...
        $select = $connection->select()
                ->from(['cbs' => $this->getTable('et_promotionbar_store')], 'store_id')
                ->join(
