<?php
namespace ET\Theme\Model\CssSetting;
...
class Generator 
{
    ...
    public function __construct(
        \ET\Theme\Helper\CssSetting $cssconfigData,
    ...
    public function generateWebsiteCss($type,$websiteId)
    {
    ...
    protected function generateStoreCss($type,$storeId) 
    {
        ...
        try {
            ...
            if(empty($block)) {
                throw new \Exception(__("Template file is empty or doesn't exists:".$str4));
            }
        } catch (Exception $e) {
            $this->_messageManager->addError(__('Failed generating css file:'.$store.'in'.$this->_cssconfigData->getCssConfigDir()).'<br/> Message:'.$e->getMessage());
        }
