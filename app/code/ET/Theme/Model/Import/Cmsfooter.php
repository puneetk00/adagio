...
class Cmsfooter
{
    ...
    public function importCms($type, $demo_version)
    {
        ...
        $gatewayResponse = new DataObject([
            ...
            'request_message' => __('Error during Import Footer CMS Sample Datas.'),
        ]);

        try {
            $xmlPath = $this->_importPath . $type . '.xml';
            $demoCMSxmlPath = $this->_importPath . 'demo_cms_footer.xml';
            
            $overwrite = false;
            
            if($this->_scopeConfig->getValue("footer_section/install/overwrite_".$type)) {
                $overwrite = true;
            }
