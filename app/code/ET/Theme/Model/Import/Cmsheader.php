...
class Cmsheader
{
    ...
    public function importCms($type, $demo_version)
    {
        ...
        $gatewayResponse = new DataObject([
            ...
            'request_message' => __('Error during Import Header CMS Sample Datas.'),
        ]);

        try {
            $xmlPath = $this->_importPath . $type . '.xml';
            $demoCMSxmlPath = $this->_importPath . 'demo_cms_header.xml';
            
            $overwrite = false;
            
            if($this->_scopeConfig->getValue("header_section/install/overwrite_".$type)) {
                $overwrite = true;
            }
